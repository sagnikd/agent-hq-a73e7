              ...
              <div className="flex items-center gap-5">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 flex-wrap mb-1">
                    <h3 className="font-display text-lg font-bold text-slate-900">{meta.title}</h3>
                    {configured ? (
                      ...
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-slate-50 border border-slate-200 text-slate-400 text-[10px] font-mono font-bold uppercase tracking-wider">
                        Not set
                      </span>
                    )}
                    {configured && lastTestOk === false && (
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-red-100 border border-red-300 text-red-700 text-[10px] font-mono font-bold uppercase tracking-wider">
                        <AlertCircle size={10} /> Last test failed
                      </span>
                    )}
                  </div>
                  <p className="text-sm text-slate-500 mb-1">{meta.summary}</p>
                  {configured ? (
                    <p className="text-xs text-slate-400 font-mono">
                      {svc?.masked}
                      {svc?.updated_at && <span className="text-slate-300 ml-2">· saved {new Date(svc.updated_at).toLocaleString()}</span>}
                    </p>
                  ) : (
                    <a
                      href={meta.url}
                      ...
                      className="text-xs text-primary hover:text-slate-900 inline-flex items-center gap-1 font-semibold"
                    >
                      Get a free key from {meta.urlLabel} <ExternalLink size={11} />
                    </a>
                  )}
                  {configured && svc?.last_test?.message && lastTestOk === false && (
                    <p className="text-xs text-red-600 mt-1">{svc.last_test.message}</p>
                  )}
                </div>
                  ...
                  {configured && (
                    <>
                      ...
                      <button
                        onClick={() => clear(s)}
                        disabled={busy === s}
                        className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-red-50 hover:bg-red-100 border border-red-200 text-red-600 text-xs font-semibold transition disabled:opacity-50"
                      >
                        <Trash2 size={12} />
                      </button>
                    </>
