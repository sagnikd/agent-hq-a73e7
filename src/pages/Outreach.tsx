...
export default function Outreach() {
  ...
  return (
    <>
      ...
      {/* Campaigns list */}
      {empty && (
        <GlassCard className="text-center py-16 bg-gradient-to-br from-primary/[0.04] to-purple/[0.04] border-slate-200">
          ...
          <h3 className="font-display text-2xl font-bold text-slate-900 mb-2">Your first campaign in one sentence.</h3>
          ...
          {campaigns.map((c) => (
            <GlassCard key={c.id} hover className="bg-gradient-to-br from-white/[0.02] to-white/[0.005] !p-0">
              <div className="flex items-start gap-5 p-6">
                ...
                <Link to={`/outreach/${c.id}`} className="flex-1 min-w-0 space-y-2 hover:text-slate-900">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className="font-display text-base font-bold text-slate-900 truncate">{c.name}</h3>
                    <span
                  ...
                  <p className="text-sm text-slate-600 italic line-clamp-2">"{c.query}"</p>
                  ...
                  {c.structured_query && (
                    <div className="flex flex-wrap gap-1.5 items-center">
                      ...
                      {c.structured_query.searchTerms.slice(0, 3).map((t, i) => (
                        <span
                          key={i}
                          className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-slate-50 border border-slate-200 text-slate-500 text-[11px] font-mono"
                        >
                          <Search size={10} /> {t}
                        </span>
                      ))}
...
function StatCard({
  ...
  return (
    <div className={`rounded-xl p-4 bg-gradient-to-br ${tints[tint]} border`}>
      ...
      <div className="font-display text-2xl font-black text-slate-900 tracking-tight">
        <AnimatedNumber value={value} />
      </div>
      <div className="text-[10px] uppercase tracking-widest text-slate-500 font-semibold mt-0.5">{label}</div>
    </div>
  );
}
